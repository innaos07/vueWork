export default class FetchProvider {
	...
	request (options) {
        ...
        console.log('requesr', options, options.baseUrl, options.path, this.computeQueryParams(options.query) )
		return fetch(
	...
	get (path, requestOptions) {
        console.log('fetchPosts 4', path, requestOptions)
		return this.request({ path, method: 'GET', ...requestOptions })
	}
	
	post (path, requestOptions) {
        console.log('post 4', path, requestOptions)
